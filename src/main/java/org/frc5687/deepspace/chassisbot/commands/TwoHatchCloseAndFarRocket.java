...
public class TwoHatchCloseAndFarRocket extends CommandGroup {
    public TwoHatchCloseAndFarRocket(Robot robot, boolean OffHAB, boolean left, Trajectory leftTrajectory, Trajectory rightTrajectory) {
        ...
        addParallel(new WristUp(robot.getHatchIntake()));
        addSequential(new AutoAlign(robot.getSparkMaxDriveTrain(), robot.getIMU(), left ? -30 : 30, .7, 500, 2, "aligning to rocket"));
        ...
        addSequential(new AutoAlign(robot.getSparkMaxDriveTrain(), robot.getIMU(), left ? -175 : 175, .7, 1000, 2, "aligning to loading station"));
        ...
        addSequential(new GripClaw(robot.getHatchIntake()));
        addSequential(new AutoAlign(robot.getSparkMaxDriveTrain(), robot.getIMU(), left ? -179 : 179, .7, 250, 5, "aligning to rocket"));
